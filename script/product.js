  ...
  function ProductDetails() {
    fetch("/product.json")
      ...
      .then((data) => {
        ...
        const showStorage = !showSize && Array.isArray(storages) && storages.length > 0;

        const price = Number(product.price.replace(/,/g, ""));
        const oldPrice = Number(product.oldPrice.replace(/,/g, ""));
        const discount = oldPrice && price ? Math.floor(((oldPrice - price) / oldPrice) * 100) : 0;
        ...
        document.getElementById("product-container").innerHTML = `
          <div class="about-product row g-4">
            <div class="col-md-6 product-img">
              <img id="mainImage" src="${product.image[0]}" class="img-fluid border mb-3 main-img"
            ...
            <div class="col-md-6 description">
              <h2>${product.title}</h2>
              <div class="price">
                <span class="lessPrice text-danger">${discount ? `-${discount}%` : ""}</span>
                <p>
                  ${
                    oldPrice
                    ...
                    ? `<p>Storage:
                        <span class="d-inline-flex flex-wrap gap-2">
                          ${storages
                            ...
                            <label class="btn btn-outline-secondary" for="storage-${i}">${st || "Default"}</label>
                          `
                            )
                            .join("")}
                        </span>
        ...
        allImages.forEach((imgUrl, i) => {
          ...
          img.style = "width: 70px; height: 70px; object-fit: cover; cursor: pointer;";
          ...
          if (Array.isArray(product.catalog)) {
            ...
            product.catalog.forEach((entry) => {
              Object.entries(entry).forEach(([colorKey, value]) => {
                catalogMap[colorKey.toLowerCase()] = Array.isArray(value) ? value[0] : value;
              });
            });
      ...
      .catch(() => {
        document.getElementById("product-container").innerText = "Failed to load product.";
      });
  }
